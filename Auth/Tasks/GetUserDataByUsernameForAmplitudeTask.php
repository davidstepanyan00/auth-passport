<?php

namespace App\Containers\AppSection\Auth\Tasks;

use App\Containers\AppSection\Auth\Data\Repositories\Interfaces\UserRepositoryInterface;
use App\Containers\AppSection\Auth\Models\User;
use App\Ship\Parents\Tasks\Task as ParentTask;

class GetUserDataByUsernameForAmplitudeTask extends ParentTask
{
    public function __construct(protected UserRepositoryInterface $userRepository)
    {
    }

    public function run(string $username): User
    {
        return $this->userRepository
            ->byUsername($username)
            ->with(['team.team', 'team.roles', 'team.team.subscription.plan', 'team.team.owner'])
            ->firstOrFail();
    }
}
